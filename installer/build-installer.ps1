...
$releaseDir = Join-Path (Get-Location).Path 'x64\Release'
...
# Build and package the single Release output.
& $msbuild OptiScan.slnx -t:Rebuild -p:Configuration=Release -p:Platform=x64 `
    "-p:OutDir=$releaseDir\" -m -nologo -v:m
if ($LASTEXITCODE -ne 0) { throw "Build failed." }

Write-Host "`n=== Compiling installer ===" -ForegroundColor Cyan
Write-Host "  ISCC: $iscc"
& $iscc "/DReleaseDir=$releaseDir" installer\OptiScan.iss
