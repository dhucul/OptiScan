...
bool ParseCueSheetForImport(const std::wstring& cuePath, CueImportSheet& out,
    ...
    while (std::getline(stream, rawLine)) {
        ...
        if (StartsWithKeyword(line, L"REM", rest)) {
            if (rest.find(L"OPTISCAN_PREGAP_UNKNOWN") == 0 || rest.find(L"OPTISCAN_GAPS_DISCARDED") == 0)
                return fail("pregap audio or boundaries are unavailable; an exact layout cannot be recreated");
            if (rest.find(L"OPTISCAN_PREGAP_FILE") == 0) out.imageGapFiles = true;
            continue;
        }
    ...
    for (auto& t : out.tracks) {
        ...
        if (t.hasIndex00() && t.index00File == t.index01File &&
            ...
            err = "track " + std::to_string(t.number) + " INDEX 00 occurs after INDEX 01";
            return false;
        }
    }
...
bool BuildBinLayout(const CueImportSheet& sheet,
    ...
    // The writer emits -150..-1 itself. Only that mandatory portion of a
    // first-track PREGAP is accounted for here; extra silence and INDEX 00
    // audio remain in the program image, with INDEX 01 at its real position.
    auto& firstTrack = tracks.front();
    if (firstTrack.pregapFrames > 0 && firstTrack.pregapFrames < 150) {
        err = "Track 1 PREGAP is shorter than the mandatory 150-frame initial pause";
        return false;
    }
    if (firstTrack.pregapFrames > 0)
        firstTrack.pregapFrames = firstTrack.pregapFrames > 150 ? firstTrack.pregapFrames - 150 : 0;
    const size_t firstFile = firstTrack.hasIndex00() ? firstTrack.index00File : firstTrack.index01File;
    if (firstFile > lastFile || firstFile >= fileStart.size()) {
        err = "track 1 begins outside the available audio files";
        return false;
    }
    // Retain all source audio before INDEX 01, including leading silence.
    fileStart[firstFile] = 0;
    ...
    if (placements.front().binIndex01 > 0) {
        placements.front().hasIndex00 = true;
        placements.front().binIndex00 = 0;
    }
